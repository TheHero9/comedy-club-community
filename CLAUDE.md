...
- 🚨 **`clerk_api.fetch_user` is returning HTTP 403 in production, and the failure is INVISIBLE by design.** It fails soft on purpose - the JWT was already cryptographically verified, so a Clerk API outage must never block a genuine sign-in - which means the only trace is a `WARNING` in the logs. Discovered 2026-08-16.
  ...
  - 🔍 **403, not 401 or 404.** The key is accepted but not authorised for that resource - classically an `sk_test` key against a production-instance user, or a restricted key without `users:read`. 401 would mean a bad key; 404 would mean the id is in another instance. Check `CLERK_SECRET_KEY` on **every** service that calls it (`api` AND `celery-worker`), not just one - they are separate Railway variables and can drift apart.
