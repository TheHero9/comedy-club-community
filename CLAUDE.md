...
- 🚨 **Contact details live in `lib/contact.ts`, never in `lib/copy.ts`.** A handle and an address read identically in both locales, so a dictionary entry means two duplicates that drift; **the label around a value is copy, the value is data.** An empty string means "not published" and the row is dropped rather than rendered as a dead link. 🔒 The repo is public and these ship in the bundle, the HTML and git history forever - only ever put in there what is meant to be handed out. Rendered on `/me` in BOTH branches: the signed-out one returns early, and someone who cannot sign in is exactly who needs the address.
- ⚠️ **`lucide-react` v1 removed every BRAND icon**, so `import { Instagram }` fails typecheck. `HelpContact` draws it as an inline SVG at lucide geometry. That is the exception, not a licence to hand-roll icons lucide still ships.
