...
- 🚀 **A push to `main` now deploys BOTH halves - web via Vercel, API via `.github/workflows/deploy-api.yml`.** Full write-up and the rejected alternatives: [`specs/10-deployment/02-auto-deploy.md`](specs/10-deployment/02-auto-deploy.md).
  - ⚠️ **It needs the `RAILWAY_TOKEN` secret** (a project token for `comedy-club-community` / production). Without it the workflow fails loudly rather than skipping. Until it is set, the API must still be deployed by hand.
