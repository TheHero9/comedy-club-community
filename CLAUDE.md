...
### Images: NOTHING on this site is optimized by Vercel

> 🔬 Full write-up in [`specs/24-image-delivery/01-the-optimizer-that-ran-out.md`](specs/24-image-delivery/01-the-optimizer-that-ran-out.md).

- 🚨 **`images.loaderFile` points at `apps/web/lib/image-loader.ts`, which returns Google CDN URLs directly. `/_next/image` is never requested and no image transformation is ever billed.** Deleting that one line silently restores the paid optimizer, and the symptom only appears once the allowance runs out again - i.e. long after the change.
  - 🔍 **The incident it came from.** On 2026-08-22 every thumbnail on the live site was gone with nothing deployed and nothing erroring. Google was serving the files normally; `/_next/image` was answering **`402 Payment Required`** (`X-Vercel-Error: OPTIMIZED_IMAGE_REQUEST_PAYMENT_REQUIRED`) to every width not already in the edge cache. The handful still rendering were stale cache entries days old - so it read as "some thumbnails broke", not "the optimizer is refusing".
  - 🚨 **This is the SAME rule the ingestion side already states** ("never mirror a thumbnail, Google's CDN serves it free and forever"), which was written about storage cost and should always have been about **any** per-image cost. ~1,961 episodes are ~1,961 *distinct* source images, so nothing amortises: one full crawl is ~9,800 transformations. `GridInteraction`'s hover preview requests a **second** URL per episode (`hqdefault` vs the card's `maxresdefault`), so a mouse sweep across the flagship channel's 1,225 cells burns hundreds more.
- 🚨 **The loader's thresholds are deliberately NOT the source widths - do not "correct" them to 320/480.** YouTube's buckets are 320, 480 and 1280 wide with nothing in between, so an honest ladder sends a 390px phone at 2x (~780px) straight to the 1,280px file at **208 KB per card**. Declaring the 480x360 file as the **828w** candidate caps a card grid at ~21 KB per image. Measured: `mqdefault` 320x180/14 KB, `hqdefault` 480x360/21 KB, `sddefault` 640x480/77 KB, `maxresdefault` 1280x720/208 KB, all present on 40/40 sampled episodes.
- 🚨 **Above the last threshold the loader returns the URL it was GIVEN, never `maxresdefault` by name.** Only `mqdefault` and `hqdefault` are YouTube guarantees; `ingestion/thumbnails.py` already HEAD-probes `maxresdefault` per episode and stores `hqdefault` when absent, so the incoming URL is the largest size **known to exist for that video**. Naming a bigger one is a guess, and a wrong guess renders the stripe placeholder with no way to find out.
- ⚠️ **`deviceSizes` has exactly three rungs because the loader has exactly three outputs, and a fourth is a DUPLICATE URL in every srcSet on the site.** It was `[320, 480, 828, 1280]` for one build and 480 emitted the same `hqdefault` as 828. Add a rung only when the loader gains a real bucket. `tests/image-loader.spec.ts` pins the two files against each other.
- ⚠️ **Avatars are the one non-thumbnail case and they only ever SHRINK.** Their size is a `=s480` suffix on an opaque content hash, re-derived per request (`=s64` for a 64px tile). Asking Google for a size above the stored one buys an upscale of a file we already have; a banner's `=w1707` token is left alone.
...
- 🚨 **Excluding shorts is NOT enough - promo clips live on the `videos` tab too.** A manual pass over all 1,962 rows on 2026-08-15 removed **100** stand-up excerpts, trailers, show announcements and channel-update clips that had been ingested as episodes. Nothing in the metadata separates them from a real episode: duration is a hint, not a rule (a 1:45 news bulletin is real, an 11:55 "last day in the old studio" is not), so this is a human judgement and always will be.
  ...
  - 🚨 **A deletion does not stick while a sync is scheduled.** Ingestion is `update_or_create(youtube_id=...)`, so the nightly `sync-channels-daily` would have recreated all 100 the next morning and silently undone the pass. That entry is now **unscheduled** in `config/celery.py`; re-enabling it without an exclusion list first WILL resurrect them. ⚠️ **There is no `manage.py sync_channels` command** (verified 2026-09-25) - the only entry points are the Celery tasks, and the sync has never run in production. Plan: `specs/26-new-episode-pipeline/01-analysis.md`.
