...
| 2026-08-15 | 🧹 **100 videos that were never podcast episodes removed from the catalogue - local AND production.** Stand-up excerpts, trailers, show announcements and channel-update clips uploaded to the `videos` tab, so ingested like any other episode. Excluding Shorts was never enough. Nothing in the metadata separates them (a 1:45 news bulletin is real; an 11:55 "last day in the old studio" is not), so it was a human pass over all 1,962 rows via a new `export_review_page` - one self-contained HTML file, click-to-mark, `localStorage`-persisted so the pass survives a closed tab. `remove_episodes` did the deleting from Postgres and BOTH Meilisearch indexes, dumping every row to JSON first. Cascade: 100 episodes + 99 transcripts + 119 segments + 51 topic links + 15 orphaned topics. **1,962 → 1,862, and local/prod now answer identical totals** (`пица` 21/21, `еврвизия` 6/6, `стендъп комедия` 1169/1169). |
| 2026-08-15 | 🚨 **A deletion does not stick while a sync is scheduled.** Ingestion is `update_or_create(youtube_id=...)`, so `sync-channels-daily` at 04:00 would have recreated all 100 the next morning and silently undone the review - in prod, where nobody would have been watching. `sync-channels-daily` is now **unscheduled** (owner decision); new episodes are pulled deliberately with `manage.py sync_channels`. The purged ids live at `podcast/data/removed-episodes.txt` in the repo, so they ship in the image and the same command runs identically in every environment. |
| 2026-08-15 | 🐛 **`transcript_index.ensure_index` self-deadlocked, and the symptom pointed at the wrong system.** `ensure_index_once` calls it while holding a non-reentrant `threading.Lock`, and it took the lock again - so any process whose first transcript-index write went through `ensure_index_once` hung forever. The episodes index hit this exact bug on 08-14 and was fixed with a bare assignment plus a comment; the transcript module was a copy that never got it. **Meilisearch completed the underlying task in 9ms and stayed green the whole time**, so the queue was empty, `/health` was fine, and it read as a slow search server. Found because it hung `remove_episodes` twice. Also learned: the per-episode index helpers are for the incremental path - looping them over 100 episodes is 200 HTTP round trips; batch with one `delete_documents([...])` and one `IN [...]` filter. |
| 2026-08-16 | 🔒 **Owner account granted full admin in production** via `manage.py grant_admin`, run as a Railway `preDeployCommand` on a new celery-worker deployment. Admin is TWO switches - `UserProfile.role` governs the API, Django's `is_staff`/`is_superuser` govern the admin site - and the command sets both, is idempotent (already-granted exits 0, so it survives re-running on every deploy) and has `--revoke`. Verified: `role=member [plain]` → `role=admin [superuser, staff]`, `/admin/` returns 200. |
| 2026-08-16 | 🚨 **`clerk_api.fetch_user` has been returning HTTP 403 in production since launch, and nothing reported it.** It fails soft by design - the JWT is already cryptographically verified, so a Clerk outage must not block a real sign-in - so the only trace is a log WARNING. **The symptom lives in the data instead:** with no Backend API enrichment, `provision_user` gets empty strings for name/email/username and creates the account with a **blank email and the raw Clerk `sub` as its username**. Production's only real account is exactly that shape - which is the very bug `clerk_api.py` was written to fix, still happening because the lookup it depends on never succeeded. 403 means the key is accepted but unauthorised for the resource (an `sk_test` key against a live-instance user, or a missing `users:read` scope), so `CLERK_SECRET_KEY` needs checking on **both** `api` and `celery-worker` - separate Railway variables that can drift. **Owner action required; not fixable from code.** |
| 2026-08-16 | 🔍 **Found it because a guardrail refused to guess.** `grant_admin --verify-email` asks Clerk which address owns a `clerk_user_id` and fails CLOSED - the opposite of the sign-in path, because here the lookup IS the verification rather than an enrichment. It refused twice, which is what surfaced the 403. The grant was then completed by elimination: production holds exactly ONE `user_`-prefixed account (the rest are `dev_` fixtures, the Django superuser and `auto-labeller`), and the owner is on record signing in on the live site, so that row could not be anyone else. Also fixed: the failure log said only `HTTPError`, which cannot distinguish 401 from 404 from 403 - the status code is now logged, the response body still is not (it echoes account data). |
| 2026-08-16 | ✅ **Clerk 403 root cause confirmed: an `sk_test_` key pointed at the PRODUCTION instance.** `grant_admin --diagnose` printed it outright - `CLERK_SECRET_KEY: sk_test_… (50 chars)` against `CLERK_ISSUER: https://clerk.comedycommunity.club`. Clerk's dev and production instances hold **separate user tables**, so a dev key has no rights over a production user; that is why Clerk answers 403 rather than the 401 a bad key would get. Swapping celery-worker's key to match `api` did NOT fix it, which is itself the finding: `api` was on a test key too, so both services matched and both were wrong. **Owner action: set the production instance's `sk_live_` key on `api` AND `celery-worker`.** Until then no account gets a name or an email - `provision_user` receives empty strings and falls back to the raw Clerk `sub`. |
| 2026-08-16 | 🎯 **Community participants, moment logging and problem reporting shipped (spec 14, `c8be8c0`).** Members suggest who took part, log a moment, and report a problem; moderators verify in Django Admin and the member sees the decision. **Participant proposals live in their OWN table** - a `status` column on `EpisodeParticipant` would have forced five existing readers (serializer, `?person=` filter, person endpoint, search fallback, Meilisearch builder) to learn to filter it, and the one forgotten would publish unverified data into search. Consequence: `EpisodeParticipant` needed **no schema change at all**. **User input never creates a `Person`** - the auto-captions mishear Тонката as Донката and the same performer is also Тони, so free-text personas would split one filmography across three pages. Three pre-existing bugs fixed en route: `copy.episode.role()` rendered a PRODUCER as "guest"; the report bulk action used `queryset.update()` and silently dropped `resolution_note`, which IS the feedback loop; and `_report_out`/`target_display` would have raised on the first target-less report. 1732 backend + 210 web + 381 E2E green; verified by hand in a browser and again on production (`/api/health` reports `c8be8c0`). |
| 2026-08-16 | ⚠️ **`router.refresh()` does not invalidate the server fetch cache**, so the moments and cast lists became client components. Both are fetched with `revalidate: 60`; a member would have added a moment, seen nothing for up to a minute, and concluded the button was broken - the same stale-fetch class of bug already recorded for the ratings grid. The server still renders the initial list, so first paint and crawlers are unchanged. |
| 2026-08-16 | 🚨 **The whole site felt slow because Next had been silently opted out of prefetching, and none of the waiting was ever shown.** Reading the locale cookie makes every route dynamic (a priced trade from spec 11 - but only its RENDER cost was priced), and **Next skips prefetching dynamic routes entirely unless they carry a `loading.tsx`**. The app had exactly one, on `/status`. So every click on the site blocked on a full server round trip with the previous page still on screen and no indication anything had happened - which the docs describe in as many words as "the impression that the app is not responding". The API was never slow; every route answers in tens of ms. 🚨 **The reason it hid for months is a correct rule that got over-applied**: "never `app/loading.tsx`" (which turns every `notFound()` into a soft 404, and cost a day on 08-08) had been read as "loading files are dangerous", so none were ever added anywhere. The real rule is narrower - never at the root, and never ABOVE a segment that can `notFound()`. |
| 2026-08-16 | 🎯 **Seven `loading.tsx` boundaries added, and `/channels` + `/me` needed route groups to get one at all.** A loading file covers its segment AND every child, so one at `app/channels/` would wrap `/channels/[slug]` and one at `app/me/` would wrap `/me/[list]` - the same soft-404 bug with a smaller blast radius. Both index pages moved into `(index)` / `(overview)` groups so the boundary scopes to the listing; the URLs are unchanged. The three routes that genuinely cannot take a boundary (`/e/[youtubeId]`, `/channels/[slug]`, `/me/[list]`) get `NavProgress` instead - a `useLinkStatus` top bar rendered INSIDE each Link, invisible for the first 120ms so a fast navigation shows nothing. **Verified with the check CLAUDE.md demands: all four bad URLs still answer a hard 404, all seven good ones 200**, and `e2e/status-codes.spec.ts` 2.6 now asserts those two directories stay clean and adds `/me/not-a-list`. |
| 2026-08-16 | 🐛 **The "it goes back to the home page for a second" report was literal.** `SearchOverlay.go()` called `onOpenChange(false)` and *then* `router.push` - so the sheet closed first and the page underneath was visible for the entire ~2s that `force-dynamic` + two Meilisearch round trips take. The push is now wrapped in `useTransition` and the sheet holds, with a spinner on the submit button, until the new route commits. |
| 2026-08-16 | 🧪 **Adding a boundary made `/search` STREAM, which made two E2E tests fail in a way that looked like product bugs and was not.** React streams a completed Suspense boundary into a `<div hidden>` and moves it with an inline script, so the document briefly contains the results **twice** - surfacing as a doubled result id and a strict-mode violation on `getByText`. Fixed with a shared `waitForResults()` that waits for **exactly one** `<h1>`; the heading is the right anchor because it renders for every query including one that matches nothing, so waiting on a result region would hang forever on the empty case. Re-run twice over both viewports: 52/52, no flakes. |
| 2026-08-16 | ⚡ **`/search?q=ергена` went 180 → 190 KB, with BOTH halves measured rather than assumed.** Removing `NavProgress` and rebuilding measured **181.1 KB - still over the old ceiling** - so only 1.8 KB is new (26 spans x ~45 bytes, doubled by the RSC flight) and 22.6 KB was already there. The page renders 24 cards, inside the budgeted 26, so this is ordinary community-data growth per card, not card count. Ceiling raised with that reasoning recorded; the next real lever is pagination, not a bigger number. |
| 2026-08-16 | 🎯 **Moments: the timestamp is now optional, and can finally be typed on a phone.** The field carried `inputMode="numeric"` - a **digits-only keypad with no colon key at all**, so the one separator the format requires was unreachable on the device most of this audience uses. `maskTimestampInput` now inserts it (type `13029`, read `1:30:29`), grouped from the right so every prefix of a real time is itself a real time. ⚠️ It deliberately does NOT validate: `475` becomes `4:75` and is refused, because reinterpreting it as 5:15 is the silent rewrite the strict-60 rule exists to prevent. `Moment.timestamp_sec` is nullable (**NULL, not 0** - zero is a real timestamp meaning "the very start"), `deep_link` returns None and the row renders as a `<div>` rather than an `<a>`, and notes sort last. `resolve_timestamp` gained `required=False` which only the moment endpoint passes, so every other caller keeps the strict behaviour. |
| 2026-08-16 | 🎨 **Moments split into "Your moments" and "From everyone else", capped at 8 rows per group.** Split by ownership from `my_moment_ids` on the authed call, never from `author` (a display name two members can share). Headings appear only when both groups exist - "From everyone else" above every moment on an episode you have never touched is wrong, not just noisy. |
| 2026-08-16 | 🎯 **Two cast roles added - `regular` and `offcamera` - and adding them exposed that `approve` never validated the role at all.** Django does not enforce `choices` at the DB level, so a moderator's arbitrary string reached the column and `copy.episode.role()` would render it as "guest": a wrong answer rather than a missing one. Propose validated; approve did not, so **the unvalidated path was the privileged one**. Both now go through one helper derived from the model's own choices. |
| 2026-08-16 | 🚨 **A saved display name survived about one second, and both halves returned 200.** `display_name` had two writers and no owner: `PATCH /api/me` wrote what the member typed, and `provision_user` refreshed "the fields the identity provider owns" on **every authenticated request**. Signing in with Google means Clerk always has a name to supply, so the refresh always had something to write and silently reverted the edit before the member could navigate back. Owner's report: *"I click save, I get 'profile saved', then I go back and it's the same one that it was before."* Fixed with `UserProfile.display_name_is_custom`, checked **before** the external-id repair branch so that repair cannot become a back door into a chosen name. 🚨 Clearing the field un-sets the flag - `""` means "go back to my Google name", not "leave me permanently nameless" - and the flag tracks the NAME, not "this member used the form", or every member who ever set a handle would have their display name frozen. |
| 2026-08-16 | 🚨 **Three endpoints had been shipped with no caller, and the pattern is worth naming: an endpoint with no reader is not a feature.** `GET /api/reports` and `POST /api/reports/{id}/resolve` have existed since wave 13 - so a filed report went into a table nobody could see from the product (*"where should I see it as an admin? I don't see anything anywhere"*), and `resolution_note`, the reply the reporter is already shown, had a reader and no writer. An approved proposal left the queue and appeared nowhere, so the only evidence of a decision was the absence of the row. And `DELETE /api/reports/{id}` had no way to be reached. All three now have UI: a moderator report queue with resolve/dismiss + note, a `/moderation/participant-proposals/reviewed` timeline carrying the new `reviewed_by`, and a withdraw control on the reporter's own list. |
| 2026-08-16 | 🚨 **A content type and a row id are not a report.** The payload could only ever have said "comment 41", which cannot be acted on without leaving the page - most of why nothing was built on that endpoint for a whole wave. `ReportOut` gained `target_label`, `target_youtube_id`, `reporter` and `resolved_by`, resolved in **bulk** (at most two queries per distinct content type) rather than dereferencing the generic FK per row, which is the exact N+1 that produced a 102-query search fallback here. ⚠️ A deleted target degrades to an empty label instead of raising - deleting the reported row is frequently the *response* to the report, so the queue has to survive its own outcome. |
| 2026-08-16 | 🎯 **The proposal queue groups by episode AND proposer**, because five suggestions for one episode are one submission, not five decisions (*"they'll suggest like 5 people for one episode, I should see them as a batch"*). Grouped by proposer too: two members proposing different casts for the same episode are two judgements, and merging them would hide a disagreement behind one "approve all". That button is disabled until every row has a persona picked - a partial batch would apply some and 422 the rest with nothing on screen saying which. Writes go sequentially, not `Promise.all`: five simultaneous writes is the shape most likely to trip the API-wide throttle. 🚨 The persona picker owns its **own** query rather than sharing the roster's - filtering the roster to find someone would otherwise empty every "approve as" dropdown on the page - and it states its 100-row cap, because a silently truncated picker reads as "that person does not exist yet" and invites a duplicate. |
| 2026-08-16 | ⚠️ **`GET /api/people` took `limit` alone, which is a cap and not pagination** - past it the remaining personas were unreachable and every caller rendered its slice as though it were the catalogue. Now takes `q` and `offset`. The ordering gained `id` as a final tiebreaker: personas with equal appearance counts had no stable order, and an unstable sort under offset paging silently drops and duplicates rows between pages. |
| 2026-08-16 | 🎯 **Cast roles cut to three - `regular`, `guest`, `offcamera` - with `regular` the default** (owner: *"we don't need host and co-host, just remove them"*). `host`/`cohost` ranked the people who are simply on the show every week; `producer` was a credit rather than a presence. 🚨 **Migration 0011 REMAPS the removed keys rather than trusting the table to be empty**: Django does not enforce `choices` in the database, so a leftover `host` would survive and render through the web's `?? regular` fallback - a wrong answer, not a missing one. 🐛 Found while removing them: the episode page picked its "similar episodes" guest with `person.role !== copy.episode.roleHost`, comparing a wire value against a **translated label** - it worked in English and matched the first participant in Bulgarian. |
| 2026-08-16 | 🐛 **A moment appeared to vanish and then come back on its own.** It was never lost: `listMoments` was fetched with `revalidate: 60`, so returning to the episode page re-rendered from a response captured *before* the write. Moments, cast and comments now use `LIVE_CACHE` (`no-store`) - these are the reads a member can change from the page they are looking at. `router.refresh()` is not a fix (it does not invalidate the server fetch cache) and neither is a write hook (the writes go browser → Django, so Next never learns of them). Measured after: `/e/D2yanlVBl-s` 85.2ms median / 89.8 KB, inside budget; payload is unchanged because caching is not what the budgets measure. |
| 2026-08-16 | 🎨 **The moment timestamp hint now explains the masking, and the "yours" chip is gone.** The mask was working and **nothing on screen said so** - the hint mentioned only that blank was allowed, so a digits-only keypad read as broken (*"I don't see the two dots, this is absurd"*). It now reads "Type digits only - 13029 becomes 1:30:29." The "yours" pill was deleted: the row already names its author under the label, and a delete button only ever renders on a row you own, so the chip restated what the control beside it already proved. |
| 2026-08-18 | 🚨 **The app had never been opened on the engine iOS ships, and 386 green tests could not have told you otherwise.** Both Playwright projects were Desktop Chrome - the one named `mobile` is Chrome resized to 390x844. Swept with real WebKit (26.5) at an iPhone viewport over eleven routes plus 320/375/768px and landscape. **The app itself is sound**: zero console errors, zero page errors, zero failed requests, no sideways scroll at any width, Cyrillic in the right faces, and no iframes or `<video>` to go wrong. One serious bug, two smaller ones. |
| 2026-08-18 | 🚨 **Every form control in the product zoomed the viewport on focus and never zoomed back.** Safari zooms when a focused control is under 16px and does not undo it, so the page stays horizontally scrolled for the rest of the session - it reads as "the layout broke when I tapped the box". Measured on WebKit: 13px on the moment composer, cast proposer, `/me/people`, report dialog and both moderation queues; 15px on the profile and membership editors. **The rule preventing this already existed, in `@layer base`, and was inert** - cascade layers beat specificity, so `.text-small` (components) and `text-[15px]` (utilities) outranked it no matter how the selector was written. The comment claimed 16px and the browser reported 13. Fixed by moving it **outside every layer**, scoped to `(pointer: coarse), (max-width: 767px)` so the desktop type scale is untouched. All 15 controls re-measured at 16px, no layout moved. |
| 2026-08-18 | 🔍 **Nothing in the repo could have caught it, and that is the transferable part.** `typecheck`/`lint`/`build` do not evaluate the CSS cascade, Vitest has no browser, and **Chromium does not zoom on focus** - so both "mobile" projects were structurally incapable of seeing it. Same shape as the three bugs that justified E2E in the first place: a static gate green over a broken app. |
| 2026-08-18 | 🎯 **A third Playwright project, `ios`, now runs `devices["iPhone 14"]` (WebKit) over one file - 12 tests, ~11s.** Scoped with `testMatch`, and `testIgnore`d from the other two, because running the whole suite a third time buys coverage they already give; the desktop project would also correctly FAIL the 16px assertions. **Verified against the bug rather than merely run:** disabling the CSS fix and rebuilding fails exactly the four focus-zoom tests and nothing else. The helper also refuses to pass on an empty selector result - which fired for real while authoring, because `/me/people` renders a skeleton until the `me` query resolves. |
| 2026-08-18 | ⚠️ **Two smaller iOS-only faults fixed.** (1) `<meta name="theme-color">` was a single static dark value and **Safari tints its own toolbars with it**, so a member on the light theme got a cream page framed in near black; `ThemeColorMeta` now rewrites it from `resolvedTheme`. Deliberately NOT a `prefers-color-scheme` pair - `enableSystem={false}`, so the theme is a stored choice unrelated to the phone's setting, and keying off the system would be the same bug reversed. (2) "Add to Home Screen" opened inside Safari chrome; `appleWebApp` is now declared, with `statusBarStyle: "black"` rather than `black-translucent`, which would put the 54px header behind the clock. |
| 2026-08-18 | ⚠️ **The three header controls present on every route were 28/34/38px against a 44px touch minimum.** Grown with a transparent pseudo-element (`.tap-target`) so the hit area clears 44px and **not one drawn pixel changes**; confirmed by hit test, not inspection - `elementFromPoint` 3px outside the logo and avatar returns the link. ⚠️ The expansion is invisible, so nothing shows you when two collide and one starts eating the other's taps: measured, settings grows +3 a side and the avatar +5 across an 8px gap, so they meet exactly. Sub-44px chips and inline text links inside dense content were left alone and **written down** - expanding those is a design change, and in tight vertical rhythm an invisible 44px box would steal a neighbour's taps. |
| 2026-08-18 | ✅ **Two things were already right for the right reasons, and both are recorded so they are not "fixed".** The viewport does NOT set `viewport-fit: cover` - without it iOS insets the layout viewport, so the fixed bottom nav and action bar clear the home indicator by themselves, and turning it on would **bury both bars under the indicator**. And sheets are sized in `dvh`, not `vh`: `vh` in Safari resolves against the large viewport, so a `vh` sheet exceeds the screen for exactly as long as the toolbars are expanded. |
| 2026-08-18 | 🧪 **404 E2E (desktop + mobile + ios), 400 passed, 4 skipped, 0 failed; 227 Vitest; 35 perf budgets green.** Flagship channel page 630.6 KB, in line with the post-flow-grid figure. Full write-up: `specs/19-ios-safari-compatibility/01-findings.md`. |
| 2026-08-18 | 🚨 **"Transcript search is unavailable right now" on production, with every layer healthy.** The API logged `MeilisearchTimeoutError ... read timeout=5`; Meilisearch's own log for the SAME request said `POST /multi-search status_code=200 time.busy=5.13ms time.idle=14.5s` - **it searched in five milliseconds and then took fourteen and a half seconds to answer**, nine seconds after the client hung up. Both halves of `/search` fire together, and in that instant the 1,862-document `episodes` query returned in **14.6ms** while the 61,452-segment transcript one stalled: small index resident, big index paged back off the Railway volume. **A healthy server and a resident index are different claims** - `is_available()` had passed 30 seconds earlier and would have passed again. Scope measured, not assumed: **one** occurrence in two days of logs (the owner's own query), and a re-probe minutes later answered `Царичина` in 0.42s. Same region, 0.5% CPU, nothing overloaded. |
| 2026-08-18 | 🎯 **Fixed by keeping the index warm, NOT by raising the timeout.** `podcast.warm_search_indexes` runs on Beat every 4 minutes: one `multi_search`, one `count_only` per index - which walks the posting list (the paging-in we want) while retrieving a single document (the part we do not). 🚨 **Two ways a keep-warm task silently warms nothing**, both now pinned by tests: 🇧🇬 a stop-word query is erased at index time so it matches nothing, touches nothing and still logs "ok" (`WARM_QUERY` is asserted against `STOP_WORDS`); and timing `processingTimeMs` instead of wall clock would have called the 5ms-inside-14.5s request healthy. It reports per-index `totalHits` so a WIPED index is distinguishable from a healthy one, never raises and never retries. `MEILI_TIMEOUT` stays at 5s - a longer one only converts a wrong answer into a wrong answer that also holds a gunicorn worker. ⚠️ **Probabilistic and labelled as such:** it makes the index unlikely to go cold; if the banner returns with this running, the next suspect is container scheduling, which is a hosting question. |
| 2026-08-18 | 🎨 **The ratings grid stacks its years newest-first** (owner: *"otherwise it's a bit strange every time to scroll at the bottom to see the current year"*). The flagship channel now leads with 2026 instead of 2016, eleven blocks down. **Only the vertical stack reversed** - inside a block the year still reads oldest to newest, and the transposed mobile table and `YearSparkline` keep the API's order because on a HORIZONTAL axis a right-to-left timeline is the bug. 🚨 The one silent failure mode is renumbering: `seasonCells(grid, seasonIndex)` is keyed by position in the API array, so `seasons.slice().reverse().map((s, i) => …)` hands every block a DIFFERENT year's episodes while every cell still renders and every count stays plausible. `flowSeasons` carries the original index; `e2e` 3.1c reads `data-year` from the DOM and asserts a strictly DESCENDING run rather than comparing against a derived list. |
| 2026-08-18 | 🧪 **402 E2E passed / 4 skipped / 0 failed, full pytest green, 231 Vitest, 35 perf budgets green, no migration drift.** Flagship channel page 630.6 KB - unchanged, since reordering sections moves no bytes. Full write-up: `specs/20-search-index-warmth/01-the-index-that-went-cold.md`. |
